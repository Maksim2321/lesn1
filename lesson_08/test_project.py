...
def test_positivProjectGet(create_project):
    ...
    response = requests.get(bURL+'api-v2/projects/'+project_id, headers=headers) 
    assert response.status_code == 200
    assert response.json()["id"] == project_id

def test_negativeProjectGet():
    response = requests.get(bURL+'api-v2/projects/999999999', headers=headers)  
    assert response.status_code == 404

def test_positivProjectPut(create_project):
    project_id = create_project
    payload = {"title": "Updated Project"}
    response = requests.put(bURL+'api-v2/projects/'+project_id, json=payload, headers=headers)  
    assert response.status_code == 200

def test_negativeProjectPut():
    payload = {"title": "Fail Update"}
    response = requests.put(bURL+'api-v2/projects/999999999', json=payload, headers=headers) 
    assert response.status_code == 404